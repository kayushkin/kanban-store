...
// misspelled owned KANBAN_ variable stops the new binary at boot.
...
func TestTheLiveProcessEnvironmentBuildsARegistry(t *testing.T) {
	...
	// New quotes a value it cannot parse, and only the port
	// can fail to parse: every secret is a string (TestEverySecretIsAString),
	// and Load names a variable and never its value.
	...
	if _, err := Load(registry); err != nil {
		t.Fatalf("the new binary would refuse to start in this environment: %v", err)
	}
