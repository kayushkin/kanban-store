...
//
// It exists because kanban-store has no auth and listens on *:8305, so the
// length of a 400 body was the caller's to choose. Measured against the deployed
// binary on 2026-09-07, with a 5 000-byte value at each site and the same
// request repeated with a 10-byte one:
//
//	PATCH /api/boards/{id}        business_hours.tzid       5 050 bytes    70
//	PATCH /api/boards/{id}        business_hours.days[0]    5 055          65
//	PATCH /api/boards/{id}        business_hours.start      5 036          46
//	POST  /api/cards/{id}/events  kind                      5 077          87
//	PUT   .../priority-levels     duplicate label           5 033          43
//
// so the caller chose about 4 990 of those bytes at every site. Eight sites in
// internal/model/activity.go behave this way; they are all the same statement
// shape, an %q of a value that arrived in the request body.
//
// The tzid site is the one worth explaining, because it is the only one where
// the value is rendered twice. time.LoadLocation quotes its input back inside
// its own error, so the %w half carries a second copy — capped at 255 bytes
// (NAME_MAX), after which LoadLocation refuses with a fixed "file name too long"
// instead. That cap is why bounding only the wrapped error measures almost no
// change and reads like a site that was never a defect, and why bounding only
// the value leaves 255 bytes of the caller's choosing behind. Both halves are
// bounded.
//
// The error is worth carrying at its realistic length rather than dropping. For
// an ordinary bad tzid it says something the value alone does not — "unknown
// time zone" against "file name too long" are different operator problems — and
// every such diagnosis this repo's callers can produce fits inside errorLimit.
package boundedtext
